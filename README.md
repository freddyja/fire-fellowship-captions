...
- **Brothers’ phones (Android or iPhone):** join via QR (`/?view=join&room=ABCD`) in the mobile browser — no app store install. Before the caption board, the phone asks **What language are you speaking?** (EN / ES / PT) and **What language do you want to watch?** (EN / ES / PT, or all three). One **Join** button enters the room with those choices. Spoken is the mic and Type + Send. Watch is captions on that phone only. The topic sheet stays. One mic at a time. Chrome on Android is best for live speech; iPhone can always watch and can type a caption if Web Speech is missing.
...
**Spoken vs Watch.** **Spoken** is the language that brother talks in. It sets the microphone and which language the caption is translated from. **Watch** is only the caption window on **this phone**: **EN only**, **ES only**, **PT only**, or **All three** (the default). Watch does not change the room layout, the host phone, Smart View, the TV, or any other guest. A brother can set Spoken **ES** and Watch **PT** so his phone shows one Portuguese caption window, while everyone else and the TV still show **EN | ES | PT**. Both choices are saved for this browser session and shown again the next time that phone opens the join link, before Join. The topic sheet on Join stays **EN | ES | PT** unless **Captions only** is on. Host and Join scroll as normal pages: Spoken, Watch, TV layout, Send, and Smart View are not pinned under a locked caption overlay.
