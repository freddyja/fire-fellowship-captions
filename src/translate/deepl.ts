...
/**
 * Target codes. `pt` → `PT-BR` (Brazilian Portuguese) to match the topic
 * seed language in this app. `en` → `EN-US` because DeepL no longer takes
 * bare `EN` as a target.
 */
...
export async function deeplTranslate(
  ...
  if (!key) throw new Error("DEEPL_AUTH_KEY is not set");
  ...
  const res = await fetch(deeplTranslateUrl(key, options.apiUrl || ""), {
    ...
    headers: {
      ...
      "user-agent": "Fire-and-Fellowship-Captions/0.1.0",
    },
    body: JSON.stringify({
      text: [text],
