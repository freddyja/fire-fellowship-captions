...
export function localSetupInnerHtml(opts?: { heading?: boolean }): string {
  ...
  return `
    ${heading}
    <p class="install-copy">No public internet? Run the app on a laptop. Fold and TV open that laptop on the same Wi‑Fi or phone hotspot.</p>
    ...
    <ol class="install-steps">
      ...
      <li>On the Fold and the TV, open <code>http://LAPTOP-LAN-IP:PORT</code> — usually <code>http://192.168.x.x:8080</code> — while they share that Wi‑Fi or hotspot.</li>
    </ol>
    <p class="hint" data-this-origin-wrap hidden>This device is already on <code data-this-origin></code> — use that URL on the Fold and TV if they share this network.</p>
