...
assert(iphone.active()[0].emit("en-US", "Welcome brothers") === "final", "English final publishes");
...
assert(iphone.active()[0].emit("es-ES", "Paz a ustedes hermanos") === "final", "Spanish final publishes");
...
assert(iphone.active()[0].emit("pt-BR", "Boa noite irmãos") === "final", "Portuguese final publishes");
await flush();
assert(
  iphoneSpeech.finals.join("|") === "Welcome brothers|Paz a ustedes hermanos|Boa noite irmãos",
...
assert(primed.active()[0].emit("es-ES", "mi esposa") === "final", "primed Spanish final publishes");
...
assert(primed.active()[0].emit("pt-BR", "minha esposa") === "final", "primed Portuguese final publishes");
primedSpeech.speech.stop();
await flush();
...
assert(primed.active()[0].emit("en-US", "Welcome brothers") === "final", "English still publishes after ES and PT");
...
assert(liveSwitch.active()[0].emit("es-ES", "Paz a ustedes hermanos") === "final", "Spanish spoken during a live switch publishes");
...
await flush();
assert(rejected.active()[0].emit("en-US", "Welcome brothers") === "final", "English still works before a rejected locale");
...
assert(rejected.active()[0].emit("pt-BR", "Boa noite irmãos") === "final", "Portuguese final after a rejected Spanish locale");
...
await flush();
assert(chrome.active()[0].emit("en-US", "Welcome brothers") === "final", "Chrome English final");
...
assert(chrome.active()[0].emit("es-ES", "Paz a ustedes hermanos") === "final", "Chrome Spanish final");
